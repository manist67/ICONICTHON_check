...
export default function Sidebar() {
	...
	return (
		<aside>
			...
			{ location.pathname != "/" ? <Link to={`/${id}/detail`} className={classNames({"active": detail })}>학생 현재 상태</Link> : <a>학생 현재 상태</a>}
			{ location.pathname != "/" ? <Link to={`/${id}/calendar`} className={classNames({"active": calendar })}>개인 학생 출결 현황</Link> : <a>개인 학생 출결 현황</a>}
		</aside>
	)
}